...
noise resists. The result is a clear, honestly-scoped positive: when the deployment
...
member. (v)~We use a single moderate severity per corruption and a single dataset; a severity
sweep and a second script (e.g.\ KMNIST) would test generality.
...
occlusion as well as or better than targeted augmentation. When the deployment corruption is
unknown, corruption-agnostic diversity is a label-free robustness lever; when it is known,
train for it.
