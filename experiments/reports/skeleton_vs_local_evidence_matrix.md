...
| 7 | Same backbone architecture used | `src/local_cnn/train_local_cnn.py` line 15, line 47; `src/skeleton/train_skeleton_cnn.py` line 17, line 137 | **Verified** | Both use `SimpleCNN`; skeleton run sets `in_channels` by mode. |
...
| 9 | Skeletonization uses fixed threshold `>30` | `src/skeleton/skeletonize.py` line 10 | **Verified** | Code explicitly thresholds with `img_2d > 30`. |
| 10 | Hough-augmented mode implemented with tunable params and 2-channel option | `src/skeleton/train_skeleton_cnn.py` lines 25–38, 49–67, 171–179 | **Verified** | `channel_mode` + Hough params exist and are wired through input pipeline. |
