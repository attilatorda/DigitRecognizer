...
\emph{grows} with corruption severity. The result is a clear, honestly-scoped positive: when the
deployment corruption is unknown, corruption-agnostic structural and generative diversity buys
...
\paragraph{Held-out-free combiners.}
...
\paragraph{Corruptions and protocol.}
...
\paragraph{Leave-one-corruption-out (LOCO).}
...
\section{Results}
\paragraph{Robustness over the clean-trained baseline.}
...
\paragraph{The honest limit: informed augmentation wins.}
...
\paragraph{The realistic setting: unseen corruptions (LOCO).}
...
\caption{LOCO: \texttt{conf\_all} $-$ \texttt{record\_aug\_loco} on the \emph{unseen}
         corruption (pp). Positive favours the agnostic ensemble. Left: MNIST. Right: KMNIST.}
\label{tab:loco}
\centering
\small
\begin{tabular}{@{}lrrrrr@{\hskip 1.2em}rrrrr@{}}
\toprule
 & \multicolumn{5}{c}{MNIST} & \multicolumn{5}{c}{KMNIST} \\
\cmidrule(r){2-6}\cmidrule(l){7-11}
held-out & $100$ & $200$ & $500$ & $1000$ & $2000$ & $100$ & $200$ & $500$ & $1000$ & $2000$ \\
\midrule
blur            & $+4.6$ & $+2.2$ & $-0.4$ & $+1.5$ & $+1.0$ & $+2.1$ & $+2.8$ & $+2.0$ & $+2.0$ & $+4.0$ \\
stroke\_dilate  & $+7.3$ & $+1.1$ & $-2.9$ & $+0.5$ & $+0.4$ & $+5.9$ & $+1.4$ & $-2.1$ & $+1.0$ & $+0.5$ \\
occlusion       & $+2.1$ & $+0.9$ & $+7.4$ & $+8.7$ & $+8.0$ & $+5.1$ & $+3.7$ & $+2.4$ & $+3.7$ & $+4.1$ \\
gaussian\_noise & $-2.1$ & $-14.4$& $-1.0$ & $-3.9$ & $+2.0$ & $-1.2$ & $-3.8$ & $+6.5$ & $+8.8$ & $+4.3$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Generality to a harder script (KMNIST).}
Repeating the study on KMNIST \cite{Clanuwat2018}, a harder $10$-class cursive-Japanese set,
reproduces the pattern (Table~\ref{tab:loco}, right; Fig.~\ref{fig:kmnistloco}). The agnostic
ensemble improves mCA once the diffusion member is active ($+3.8$~pp at $n\!=\!1000$, $+3.4$ at
$n\!=\!2000$; $10/10$ seeds), and under LOCO it beats unseen-corruption augmentation on
\emph{all four} corruptions at $n\!\geq\!1000$---including gaussian noise, which had resisted on
MNIST---so the effect is not specific to MNIST.

\paragraph{Severity dependence.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/fig10_track9c_loco.png}
\caption{Leave-one-corruption-out (MNIST). On blur, stroke thickening and occlusion the
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{figures/fig13_track9c_kmnist_loco.png}
\caption{KMNIST leave-one-corruption-out. The agnostic ensemble (green) matches or beats
augmentation trained on the \emph{other} corruptions (orange) on all four types for
$n\!\geq\!1000$.}
...
\includegraphics[width=\linewidth]{figures/fig9_track9c_mca.png}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}\centering
\includegraphics[width=\linewidth]{figures/fig12_track9c_severity.png}
\end{minipage}
\caption{Left: mean corruption accuracy vs.\ budget---the agnostic ensemble
(\texttt{conf\_all}) beats the clean-trained record holder everywhere; informed augmentation
(\texttt{record\_aug}) is stronger once $\geq\!100$ labels are available. Right: severity sweep
at $n\!=\!2000$---the advantage grows with severity, except under destructive noise.}
\label{fig:mca}
\label{fig:sev}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig11_track9c_attribution.png}
...
\begin{ack}
Reproducibility: \texttt{scripts/run\_track9\_robust.py} (mCA, ablation),
\texttt{scripts/run\_track9\_loco.py} (LOCO), \texttt{scripts/run\_track9\_severity.py}.
Models and corruptions: \texttt{src/track9/}. Raw results:
\texttt{track9\_robust\_results.json}, \texttt{track9\_loco\_results.json}.
\end{ack}
